...
async function main() {
  console.log("==================================================");
  console.log("  SLACK NOTIFICATIONS & RATE LIMIT ALERT VERIFIER  ");
  console.log("==================================================\n");

  // 1. Check existing user and connection
  ...
  if (!alertChannel) {
    console.warn("⚠️ Warning: #mail-scheduler-alerts not found in channel list, checking by ID...");
  } else {
    console.log(`✅ Verified #mail-scheduler-alerts found with ID: ${alertChannel.id}`);
  }

  // 3. Test setSlackNotificationChannel
  ...
  // 4. Test sendSlackTestNotification (Live Slack chat.postMessage)
  ...
  // 5. Test notifySenderHourlyLimit (Hourly rate limiter trigger)
  ...
  const nextEligibleTime = Date.now() + 45 * 60 * 1000; // 45 mins later
  ...
  // 7. Verify Outbox Records in PostgreSQL
  ...
  console.log("\n==================================================");
  console.log("  ALL SLACK NOTIFICATION TESTS COMPLETED SUCCESSFULLY ");
  console.log("==================================================");
